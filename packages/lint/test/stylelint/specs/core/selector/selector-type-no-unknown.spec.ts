...
describe('selector-type-no-unknown', () => {
  it('应该通过使用已知类型选择器的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: { 'selector-type-no-unknown': selectorRules['selector-type-no-unknown'] },
      },
      files: file,
    });
  ...
  it('应该报告使用未知类型选择器的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('selector-type-no-unknown');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/type|unknown/i);
    });
  });
});
