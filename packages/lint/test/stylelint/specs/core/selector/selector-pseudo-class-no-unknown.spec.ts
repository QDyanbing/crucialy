...
describe('selector-pseudo-class-no-unknown', () => {
  it('应该通过使用已知伪类的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: {
          'selector-pseudo-class-no-unknown': selectorRules['selector-pseudo-class-no-unknown'],
        },
      },
      files: file,
    });
  ...
  it('应该报告使用未知伪类的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('selector-pseudo-class-no-unknown');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/pseudo.*class|unknown/i);
    });
  });
});
