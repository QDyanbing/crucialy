...
describe('selector-max-combinators', () => {
  it('应该通过组合器数量在限制内的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: { 'selector-max-combinators': selectorRules['selector-max-combinators'] },
      },
      files: file,
    });
  ...
  it('应该报告组合器数量超过限制的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('selector-max-combinators');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/max|combinator/i);
    });
  });
});
