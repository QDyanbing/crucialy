...
describe('selector-max-specificity', () => {
  it('应该通过特异性在限制内的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: { 'selector-max-specificity': selectorRules['selector-max-specificity'] },
      },
      files: file,
    });
  ...
  it('应该报告特异性超过限制的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('selector-max-specificity');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/max|specificity/i);
    });
  });
});
