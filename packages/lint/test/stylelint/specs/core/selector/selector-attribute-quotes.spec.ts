...
describe('selector-attribute-quotes', () => {
  it('应该通过属性值使用引号的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: { 'selector-attribute-quotes': selectorRules['selector-attribute-quotes'] },
      },
      files: file,
    });
  ...
  it('应该报告属性值缺少引号的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('selector-attribute-quotes');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/quote|attribute/i);
    });
  });
});
