...
describe('@stylistic/block-opening-brace-space-before', () => {
  ...
  it('应该报告块开括号前缺少空格的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('@stylistic/block-opening-brace-space-before');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/block|brace|space/i);
    });
  });
});
