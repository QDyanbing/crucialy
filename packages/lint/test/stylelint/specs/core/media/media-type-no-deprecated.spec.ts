...
describe('media-type-no-deprecated', () => {
  it('应该通过没有使用已弃用 media type 的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: {
        rules: { 'media-type-no-deprecated': mediaRules['media-type-no-deprecated'] },
      },
      files: file,
    });
  ...
  it('应该报告使用已弃用 media type 的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('media-type-no-deprecated');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/deprecated|media.*type/i);
    });
  });
});
