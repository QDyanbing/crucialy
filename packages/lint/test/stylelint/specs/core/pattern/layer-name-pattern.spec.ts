...
describe('layer-name-pattern', () => {
  it('应该通过 @layer 名称使用 kebab-case 的代码', async () => {
    ...
    const { errored, results } = await runStylelintWithConfig({
      config: { rules: { 'layer-name-pattern': patternRules['layer-name-pattern'] } },
      files: file,
    });
  ...
  it('应该报告 @layer 名称不使用 kebab-case 的错误', async () => {
    ...
    ruleWarnings.forEach(warning => {
      expect(warning.rule).toBe('layer-name-pattern');
      expect(warning.severity).toBe('error');
      expect(warning.text).toMatch(/layer|name|pattern/i);
    });
  });
});
